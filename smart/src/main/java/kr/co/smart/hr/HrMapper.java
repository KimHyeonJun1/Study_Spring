...
public interface HrMapper {
   ...
   List<DepartmentVO> getListOfDepartmentWithEmployee(); //사원들이 속해 있는 부서
   List<EmployeeVO> getListOfEmployee(int department_id);//특정부서의 속한 사원목록 조회
 
   List<DepartmentVO> getListOfDepartment(); //회사의 전체 부서목록
   List<JobVO> getListOfJob();//회사의 전체 업무목록
   List<EmployeeVO> getListOfManager(int employee_id);//회사의 전체 사원목록 - 매니저
   List<EmployeeVO> getListOfManager();//회사의 전체 사원목록
   
   int countIsManager(int employee_id); //매니저 사원인지 확인(관리하고 있는 사원이 있는지)
   int countIsManagerOfDepartment(int employee_id);//부서장 사원인지확인
}
