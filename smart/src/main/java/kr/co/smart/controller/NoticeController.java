...
public class NoticeController {
	...
	public String modift(NoticeVO vo, PageVO page, MultipartFile file, HttpServletRequest request) throws Exception{
		...
		//첨부파일이 있는 경우
			...
			//원래O -> 바꿔첨부 -> DB에 저장되도록 수정정보에 담기 + 원래첨부된 물리적파일 삭제
			vo.setFilename(file.getOriginalFilename());
			vo.setFilepath(common.fileUpload("notice", file, request));
		}
		
		//화면에서 입력한 정보로 DB에 변경저장 처리
	...
	public void download(int id, HttpServletRequest request, 
		...
		NoticeVO vo = mapper.getOneNotice(id);
		//해당 글에 첨부된 파일을
		//서버의 물리적영역에서 복사해 클라이언트컴퓨터에 다운로드
	...
	public String info(int id, PageVO page, Model model, HttpServletRequest request) {
		mapper.updateReadCount(id);		//조회수 처리
	...
	public String reply(NoticeVO vo, PageVO page, MultipartFile file, @AuthenticationPrincipal LoginUser user, HttpServletRequest request) throws Exception {
