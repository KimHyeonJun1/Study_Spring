...
<ul class="nav nav-pills justify-content-center mb-3 data">
  ...
  <li class="nav-item"><a role="button" class="nav-link">기타2</a>
  </li>
</ul>
...
<jsp:include page="/WEB-INF/views/include/loding.jsp"/>

<script type="text/javascript" src="https://oapi.map.naver.com/openapi/v3/maps.js?ncpClientId=q7gvdueu9a"></script>
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=b4cc589c7556cabffcd7a4c518d3d487"></script>
<script type="text/javascript" src="<c:url value='/js/animal.js'/>"></script>
<script>
$(function(){
	$(".data li").eq(1).trigger("click");//약국조회 되도록 클릭 강제발생 시킴
})
$(document)
	...
	$.ajax({
		...
		if(response.totalCount==0){
			...
			for( var item of response.items.item ){
				tag += `<tr><td>\${item.yadmNm}</td><td>\${item.telno ? item.telno : ''}</td><td>\${item.addr}</td></tr>`
			}
		}
