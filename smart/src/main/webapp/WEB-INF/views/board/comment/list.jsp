...
<c:if test="${! empty subPage.list }">
<jsp:include page="/WEB-INF/views/include/subPage.jsp"></jsp:include>
</c:if>
...
function modifyStatus( comment ) {
	...
	//변경취소의 경우를 위한 댓글내용을 담아두기
	var content = comment.find(".content");
	var content_text = content.html().replace(/<br>/g, '\n');
	content.html( `<textarea class="form-control h-px105">\${content_text}</textarea>
...
function viewStatus( comment) {
	...
	//내용: textarea의 내용이 .content의 텍스트로 
	var content = comment.find(".content")
...
<!-- 				</div> -->
<!-- 				<div class="content">안녕하세요반가워요잘있어요다시만나요</div> -->
<!-- 				<div></div> -->
<!-- 			</div> -->
